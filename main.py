        ...
        if after.channel:
            ...
            if after.channel.id == TARGET_VC_ID:
                ...
                if len(after.channel.members) == 1:
                    other_online_members = [m.name for m in member.guild.members if m.status == discord.Status.online and not m.bot and m.name != member.name]
        ...
        # someone leaves
            ...
            if last_msg_id:
                ...
                if ACTIVE and len(before.channel.members) == 0:
                    ACTIVE = False
                    update_session_log(f"👨🏻‍💼 {manager_display_name}: 📋 MOM to be prepared by {random.choice(list(attendees)).display_name}.")
